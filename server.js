...
app.post("/api/cadastro", async (req, res) => {
  ...
  try {
    ...
    const derivedKey = await new Promise(
      (resolve, reject) => {
...
app.post("/api/login", async (req, res) => {
  ...
  try {
    ...
    if (!email || !password) {
      ...
      return res.status(400).send(`
        <h2>Informe o e-mail e a senha.</h2>
    ...
    if (parts.length !== 2) {
      ...
      return res.status(500).send(`
        <h2>Erro na senha armazenada.</h2>
        <a href="/login">Voltar</a>
      `);
    }
...
// Página inicial
app.get("/", (req, res) => {
  res.send("Central Site funcionando!");
});
